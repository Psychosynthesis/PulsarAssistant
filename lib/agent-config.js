...
function normalizeAgent(id, entry) {
  ...
  if (type === "acp") {
    ...
    if (command) agent2.command = command;
    return agent2;
  }
  if (type === "cursor") {
    const agent2 = {
    ...
    agent2.baseUrl = optionalString(entry.baseUrl)?.trim().replace(/\/+$/, "") ?? "https://api.cursor.com/v1";
    const defaultModel2 = optionalString(entry.defaultModel);
    if (defaultModel2) agent2.defaultModel = defaultModel2;
    agent2.mode = entry.mode === "plan" ? "plan" : "agent";
    agent2.autoCreatePR = entry.autoCreatePR === true;
    agent2.workOnCurrentBranch = entry.workOnCurrentBranch === true;
    return agent2;
  }
  const agent = {
    ...entry,
...
function toLaunchTarget(id, agent, model) {
  const type = agent.type ?? (agent.command ? "acp" : "openai");
  if (type === "cursor") {
    const cursorAgent = agent;
    const baseUrl = optionalString(cursorAgent.baseUrl)?.replace(/\/+$/, "") ?? "https://api.cursor.com/v1";
    const effectiveModel = optionalString(model) ?? optionalString(cursorAgent.defaultModel);
  ...
  if (type === "openai") {
    ...
    const configuredModel = optionalString(openaiAgent.defaultModel) ?? optionalString(openaiAgent.model);
    ...
    const modelsUrl = optionalString(openaiAgent.modelsUrl) ?? `${baseUrl}/models`;
    return {
      id,
...
function launchTargetsEqual(a, b) {
  ...
  if (a.kind === "cursor" && b.kind === "cursor") {
    return a.baseUrl === b.baseUrl && a.model === b.model && a.apiKey === b.apiKey && a.mode === b.mode && a.autoCreatePR === b.autoCreatePR && a.workOnCurrentBranch === b.workOnCurrentBranch;
  }
  return false;
}
